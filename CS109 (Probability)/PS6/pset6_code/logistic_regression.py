...
class LogisticRegression:
    ...
    def fit(self, train_features, train_labels):
        ...
        for i in range(self.max_steps):
            linear_comb = np.zeros(train_features.shape[0])
            for i in range(train_features.shape[0]):
                for j in range(train_features.shape[1]):
                    linear_comb[i] += train_features[i, j] * theta[j]

            predictions = sigmoid(linear_comb)

            gradient = np.zeros(theta.shape[0])
            for j in range(train_features.shape[1]):
                for i in range(train_features.shape[0]):
                    gradient[j] += train_features[i, j] * (train_labels[i] - predictions[i])
    ...
    def predict(self, test_features):
        ...
        for i in range(len(test_features)):
            ...
            for j in range(len(test_features[i])):
                z += test_features[i][j] * self.weights[j]
            prob = 1 / (1 + math.exp(-z))
            if prob >= 0.5:
                preds.append(1.0)
            else:
