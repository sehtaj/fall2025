...
def make_word_prob_map(word_list):
    ...
    for word in word_counts:
        word_probs[word] = word_counts[word] / total_words
...
def calculate_doc_log_prob(author_word_prob_map, unknown_word_count_map):
    ...
    log_prob = 0.0
    for word, count in unknown_word_count_map.items():
        if word in author_word_prob_map:
            p = author_word_prob_map[word]
            if p > 0:
                log_prob += count * log(p)
    return log_prob
...
def determine_author(word_list1, author1, word_list2, author2,
    ...
    prob_map1 = make_word_prob_map(word_list1)
    prob_map2 = make_word_prob_map(word_list2)
    unknown_counts = make_word_count_map(unknown_word_list)
    log_prob1 = calculate_doc_log_prob(prob_map1, unknown_counts)
    log_prob2 = calculate_doc_log_prob(prob_map2, unknown_counts)
